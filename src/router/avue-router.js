    ...
    this.$router.$avueRouter = {
        ...
        formatRoutes: function (aMenu = [], first) {
            ...
            for (let i = 0; i < aMenu.length; i++) {
                const oMenu = aMenu[i]
                if (oMenu.label === '首页预览'||oMenu.label === '质量规则管理'||oMenu.label === '质量评估标准'||oMenu.label === '日志') {
                } else {
                    if (this.routerList.includes(oMenu[propsDefault.path])) return
                    const path = (() => {
                        if (!oMenu[propsDefault.path]) {
                            return
                        } else if (oMenu.label === 'xxx') {
                            return '/diaodu'
                        } else if (first) {
                    ...
                    //特殊处理组件
                    const component = 'page' + oMenu.path
                    ...
                    const oRouter = {
                        ...
                        component(resolve) {
                            ...
                            if (first) {
                                ...
                                require(['../components/common/layout'], resolve)
