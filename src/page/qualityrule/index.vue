  ...
  <div class="quality-rule">
    ...
    <div class="left-menu">
      ...
      <ul @click.stop="getTreeRule">
        <li v-for="(item, index) in ruleList" :key="index" :class="{ act: index == leftmenuact }" :data-model="item.model" :data-xindex="index">
          <span class="ahove cursor" :data-model="item.model" :data-xindex="index">{{ item.value }}</span>
          <span class="nums" :data-model="item.model" :data-xindex="index">{{ item.ruleCount }}</span>
        </li>
      </ul>
      <!-- div.筛选器 -->
      ...
      <ul>
        <li class="no-pointer favorite-hover" v-for="(item, index) in paramsList" :key="index">
          <span @click="searchByParam(item)" class="ahove pointer" :data-model="item.model" :data-xindex="index">{{ item.name }}</span>
    ...
    <div class="main">
      ...
      <div class="main-header" v-show="!showSaiXuanQi">
        <div class="seach">
          <el-input v-model="keyword" prefix-icon="el-icon-search" maxlength="20" minlength="2" placeholder="请输入关键字" clearable size="mini"></el-input>
          <el-button class="seach-m-l-10" type="primary" icon="el-icon-search" size="mini" @click="seachData">查询
          </el-button>
        </div>
        ...
        <div class="btns">
          ...
          <!--                    >新增分组-->
          <!--                    </el-button>-->
          <!-- <el-button type="primary" size="mini" @click="showPattern" class="icon iconfont" :class="SwitchTableIcon"></el-button> -->
        </div>
      </div>
      ...
      <div class="SaiXuanQi" v-show="showSaiXuanQi">
        <el-row class="x-el-row">
          <el-col :span="6">
            ...
            <el-select multiple v-model="zhuangTai" placeholder="请选择" @change="changeZhuangTai">
              <el-option v-for="item in zhuangTaiList" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-col>
          <el-col :span="6">
            <span class="title">操作员</span>
            <el-select multiple v-model="operator" placeholder="请选择" @change="changeOperator">
              <el-option v-for="item in operatorselect" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-col>
          <el-col :span="6">
            <span class="title">数据源</span>
            <el-select v-model="shujuyuan" placeholder="请选择" @change="changeShuJuYuan">
              <el-option v-for="item in shujuyuandata" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-col>
          <el-col :span="6">
            <span class="title">更新日期</span>
            <el-date-picker value-format="yyyy-MM-dd" format="yyyy-MM-dd" @change="changeTime" style="width:260px" v-model="genxinriqi" type="daterange" range-separator="至"
        ...
        <div class="btns2">
          <!--
                    ...
                    -->
          <el-button type="warning" size="mini" icon="el-icon-star-off" @click="saveFavorite">收藏
          </el-button>
          <el-button type="danger" size="mini" icon="el-icon-close" @click="showShaiXuanqi">取消
          </el-button>
        </div>
      </div>
      ...
      <div class="hecha-body">
        <div class="box-null" v-if="tableList.length <= 0">没有数据</div>

        <template v-if="SwitchTableIcon === 'iconliebiao' ">

          <div v-for="(db_item, db_index) in tableList" :key="db_index">
            <div class="titlebox">
              <div class="title">{{ db_item.businessType }}数据源</div>
              <el-link :underline="false" @click="addRule">新增</el-link>
            </div>

            <div class="listbox">
              <el-row :gutter="20">
                <el-col :span="8" v-for="(table_item, table_index) in db_item.tables" :key="table_index">
                  <div class="itembox">
                    <!-- 右上角状态 -->
                    <!-- class:status1:启用；status2：报错；status0；停用 -->
                    <!-- <div class="wiayuan" :class="'status' + (table_item.status||2)">
                      <div class="neiyuan">{{ table_item.status === '1' ? '运行中' : '停用' }}
                      </div>
                      <div class="zhezhao"></div>
                    </div> -->

                    <!-- 右上角状态end -->
                    <el-tooltip class="item" effect="dark" :disabled="table_item.tablename.length <= 20 || isDisTooltip" :content="table_item.tablename" placement="top-start">
                      <div class="title">{{
                    ...
                    <div class="bottom-btns">
                      <div class="bs" @click="configRule(table_item.model,table_item.tablename)" :data-type="table_item.model" :data-table="table_item.tablename">
        ...
        <div v-else class="table-rule">
          ...
          <el-collapse @change="handleChange">
            <div v-for="(item, index) in tableList" :key="index">
              <el-collapse-item class="cb-collapse" :title=" item.businessType + '数据源'" :name="index">
                <div class="xh-collapse-item">
                  <el-table :data="item.tables" border style="width: 100%">
                    ...
                    <el-table-column prop="total" label="规则数"></el-table-column>
                    <el-table-column prop="" label="监控等级"></el-table-column>
                    <!-- class:status1:启用；status2：报错；status0；停用 -->
                    <el-table-column prop="status" label="状态">
                      <template slot-scope="scope">
                        <el-link type="success" :underline="false" v-if="scope.row.status==='1'">运行中</el-link>
                        <el-link type="danger" :underline="false" v-else>停用</el-link>
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="100">
                      <template slot-scope="scope">
                        ...
                        <el-button type="text" size="small" @click="configRule(scope.row.model,scope.row.tablename)" class="icon iconfont iconf-maintain-weihu">
                        </el-button>

                        <el-button type="text" size="small" @click="qualityReport(scope.row.model,scope.row.tablename)" class="icon iconfont iconrizhi">
                        </el-button>
    ...
    <el-dialog title="新增分组" :visible.sync="dialogVisible" width="340px" custom-class="k-dialog">
      <el-form :model="xinzengform">
        ...
        <el-form-item label="数据库" :label-width="formLabelWidth">
          <el-select v-model="xinzengform.shujuku" placeholder="请选择数据库" @change="setSelectSL">
            <!-- <el-option label="数据库一" value="shanghai"></el-option>
                        <el-option label="数据库二" value="beijing"></el-option>-->
            <el-option v-for="(item, index) in selectXinZengSJY" :key="index" :label="item.connectionCode" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="实例" :label-width="formLabelWidth">
          <el-select v-model="xinzengform.shili" placeholder="请选择实例">
            <!-- <el-option label="数据库一" value="shanghai"></el-option>
                        <el-option label="数据库二" value="beijing"></el-option>-->
            <el-option v-for="(item, index) in selectXinZengSL" :key="index" :label="item.name" :value="item.name"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getLeftTree() {
      ...
      // 默认选中第一个规则
      this.checkedRule = res.rule[0].model;
      // 通过规则查询右侧表规则配置预览
      await this.getTableOverview({
        model: this.checkedRule
      });
      // 赛选器列表 todo
    ...
    // 点击左边树，更新右边列表信息
    getTreeRule(e) {
      this.checkedRule = e.target.dataset.model;
      // 高亮当前点击的元素
      this.leftmenuact = e.target.dataset.xindex;
      // 保留点击的规则，便于后面传值使用
      leftruleval = e.target.dataset.model;
