<template>
  <div class="responsible">
    ...
    >
      ...
      <el-table :data="businessList" border style="width: 100%" class="cursor-move mar-b-10" row-key="id" ref="dragBusiness">
    ...
    >
      <!-- <div class="responsible-top mar-b-10">
          <div class="left">
            <el-select v-model="model" placeholder="上报类型" size="mini" class="select-full" @change="getTableList">
              <el-option v-for="item in modelList" :key="item.id" :label="item.value" :value="item.code"></el-option>
            </el-select>
          </div>
          <div class="right"></div>
      </div> -->
      <el-table :data="tableList" border style="width: 100%" class="cursor-move mar-b-10" row-key="table" ref="dragTable">
          <el-table-column type="index" label="序" width="50"> </el-table-column>
          <el-table-column prop="table" label="表名称"></el-table-column>
          ...
          <el-table-column prop="categoryId" label="模型分类">
            <template slot-scope="scope">
              <el-select
                ...
                class="select-full"
              >
                <el-option v-for="item in businessAllList" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </template>
          </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    handleDbCurrentChange(currentPage) {
      this.searchDb.current = currentPage
      this.getSystemDbList();
    },
    handleDbSizeChange(pageSize) {
      this.searchDb.size = pageSize;
      this.getSystemDbList();
    },
    ...
    handleMetaSizeChange(pageSize) {
      this.metaPageSize = pageSize;
      this.getSystemMetaList();
    },
    //更新状态
    updateStatus(row) {
    ...
    getTableList() {
      ...
      this.$R.getCategoryBindPage(data).then((res) => {
        this.tableList = res.data
        this.tableTotal = res.pagination && res.pagination.totalRow || 0
      })
    },
    ...
    rowDropBusiness () {
	    const el = this.$refs.dragBusiness.$el.querySelectorAll('.el-table__body-wrapper > table > tbody')[0]
      const _this = this;
      Sortable.create(el, {
        onEnd ({ newIndex, oldIndex }) {
          const currRow = _this.businessList.splice(oldIndex, 1)[0];
          _this.businessList.splice(newIndex, 0, currRow);
          let data = {
            curSort: oldIndex + 1,
            toSort: newIndex + 1,
            model: currRow.model,
          ...
          _this.$R.sortCategory(data).then((res) => {
    ...
    rowDropTable () {
      ...
      Sortable.create(el, {
        onEnd ({ newIndex, oldIndex }) {
          ...
          _this.$R.sortTable(data).then((res) => {
            _this.$message.success('排序成功！');
            _this.getTableList();
          })
        }
...
/deep/.cursor-move .el-table__row {
  cursor: move;
}
</style>
