      ...
      <div>
        <div v-if="ROWDATA">
          <el-row class="detail">
            ...
            <el-col :span="8">上报类型：{{ ROWDATA.model }}</el-col>
            <el-col :span="8">时间类型：{{ ROWDATA.jobType }}</el-col>
            <el-col :span="8">时间区间：{{ ROWDATA.businessTime }}</el-col>
            <el-col :span="8">触发时间：{{ ROWDATA.time }}</el-col>
            <el-col :span="16">责任人：{{ ROWDATA.user }}</el-col>
            <el-col :span="8">问题分析：</el-col>
          </el-row>
          <el-row v-if="ROWDATA&&ROWDATA.data&&ROWDATA.data.length&&ROWDATA.data[0]">
            <el-table size="mini" :data="ROWDATA.data" border stripe v-if="ROWDATA.ruleCode == 'main_detail'||ROWDATA.ruleCode =='source_target'">
