...
<!--                    <SqlEditor ref="sqleditor" :value="showSql" class="sql-editor-warp"/>-->
                    ...
                    <el-select class="sql-select" type="primary" v-model="sourceModel" size="small" v-if="!isTarget">
                        <el-option v-for="item in dbs" :key="item.name" :label="item.name"
    ...
    methods: {
        ...
        getRule() {
            this.$R.querySourceTarget({
                ...
                if (data != null) {
                    ...
                    this.targetSql = data.target.sql;
                    // 源表规则
                    this.sourceModel = data.source.model;
                    this.sourceSql = data.source.sql;
                    // 页面显示的sql
                    if(this.isTarget){
        ...
        save() {
            ...
            this.$R.modifySourceTarget({
                ...
                target: {model: this.targetModel, sql: this.targetSql},
                source: {model: this.sourceModel, sql: this.sourceSql}
            }).then(id => {
                this.id = id;
                this.getRule();
