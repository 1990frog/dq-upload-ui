...
router.beforeEach((to, from, next) => {
  ...
  if (store.getters.access_token) {
    if (to.path === '/login') {
      ...
      if ((from.path === "/qualityrule") && to.path === "/qualityReport")
        isFromRule = true
      store.commit("SET_FROMRULE", isFromRule)
    ...
    getSSo()
      .then((res) => {
        ...
        if (res.data.ssoEnable) {
          ...
          ssoPost++
          // 统一登录，自动登录
          var token = GetRequest().token || GetRequest().tk
          if (token) {
            loginByUsernameBase(token)
              .then((response) => {
            ...
            // 需要跳转统一登录接口
            const serverUrl = res.data.serverUrl?.replace('/sso', '/sso/Home/Logout')
            const backUrl = location.origin + location.pathname;
            window.location.replace(`${serverUrl}&clienturl=${backUrl}&RedirectM=get`)
          }
        } else {
          // 跳转到系统自带的登录页
          if (meta.isAuth === false) {
